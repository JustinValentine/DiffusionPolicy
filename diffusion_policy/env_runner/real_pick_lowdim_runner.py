...
class RealPickLowdimRunner(BaseLowdimRunner):
    def __init__(self,
            output_dir,
            dataset_dir,
            n_train=10,
        ...
        def env_fn():
            from diffusion_policy.env.wam_pick.wam_pick_env import WAMPickEnv
            from diffusion_policy.env.wam_pick.wam_lowdim_wrapper import WAMLowdimWrapper
            env = WAMPickEnv(abs_action=abs_action)
        ...
        all_init_pick_loc = np.load(pathlib.Path(dataset_dir) / "all_init_pick_loc.npy")
        ...
        for i in range(n_train):
            ...
            if i < len(all_init_qpos):
                ...
                init_pick_loc = all_init_pick_loc[i]

            def init_fn(env, seed=seed, init_qpos=init_qpos, init_qvel=init_qvel, init_pick_loc=init_pick_loc, enable_render=enable_render):
                # video_wrapper
                ...
                env.env.env.init_pick_loc = init_pick_loc
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps, desc=f"Eval WAMPickLowdimRunner {chunk_idx+1}/{n_chunks}", 
        ...
        for i in range(n_inits):
            ...
            prefix_success_map[prefix].append(last_info[i]['is_success'])
