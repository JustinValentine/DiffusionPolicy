...
from diffusion_policy.samplers.base_sampler import BaseSampler
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(len(sigmas) - 1):
            ...
            # HACK: Lazy modification to get it to work temporarily. Consider creating new class for scaling.
            c_skip, c_out, c_in, c_noise = kwargs['scaling'](sigma_hat * s_in)
            pred = model(c_in * action, c_noise.reshape(-1), local_cond=kwargs['local_cond'], global_cond=kwargs['global_cond'])  # compute denoised action
            denoised = c_skip * action + c_out * pred
