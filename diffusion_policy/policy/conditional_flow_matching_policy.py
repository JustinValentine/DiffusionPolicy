...
class ConditionalFlowMatchingPolicy(BasePolicy):
    ...
    def conditional_sample(
        ...
        **kwargs,
        ...
        # Reverse times so time starts at 0
        times = self.noise_scheduler.get_sigmas(self.num_inference_steps).flip(dims=[0])

        for t in range(self.num_inference_steps - 1):
            t0 = times[t]
            t_batch = t0 * torch.ones(B, device=self.device)

            # Compute predictions for unconditioned model
            pred_uncond = self.model(trajectory, t_batch, global_cond=global_cond_uncond)

            # Compute predictions for conditioned model
            pred_cond = self.model(trajectory, t_batch, global_cond=global_cond)

            # Combine predictions using the guidance formula
            pred = (1 - w) * pred_uncond + w * pred_cond

            dt = times[t + 1] - t0
            trajectory = trajectory + pred * dt

            if collector:
                collector.add(pred, trajectory, t0)  # Intermediate denoising steps

        return trajectory


    def predict_action(
        self, obs_dict: Dict[str, torch.Tensor], collector=None, w=1
        ...
        null_token_index = 25 #29
        null_nobs = dict_apply(nobs, lambda x: torch.full_like(x[:, :To, ...], null_token_index))
        global_cond_uncond = self.obs_encoder(null_nobs)
    ...
    def compute_loss(self, batch):
        ...
        # Justin:
        # Probability of using the null token for classifier-free guidance
        p = 0.01 # Test_1: 0.25, Test_1: 0.5 

        # Generate a mask indicating which samples will use the null token
        drop_cond = torch.rand(batch_size, device=self.device) < p

        this_nobs = dict_apply(nobs, lambda x: x[:, : self.n_obs_steps, ...])

        # Replace observations with null tokens where indicated by the mask
        def replace_with_null_tokens(obs, mask):
            null_obs = {}
            null_index = 25 #29 # Should be n_classes - 1
            for key, val in obs.items():
                val = val.clone()
                val[mask] = null_index
                val = torch.clamp(val, min=0, max=null_index) # Ensure all indices are within the valid range [0, 25]
                null_obs[key] = val
            return null_obs   

        # Apply the null token replacement
        masked_nobs = replace_with_null_tokens(this_nobs, drop_cond)

        global_cond = self.obs_encoder(masked_nobs)
