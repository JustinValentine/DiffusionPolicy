...
class RobomimicReplayKpActionDataset(BaseImageDataset):
    def __init__(self,
        ...
        if abs_action:
            raise NotImplementedError("abs_action not implemented for keypoints")
    ...
    with h5py.File(dataset_path) as file:
        ...
        for key in tqdm(lowdim_keys, desc="Loading lowdim data"):
            ...
            for i in range(len(demos)):
                demo = demos[f'demo_{i}']
                if "keypoints" in key:
                    reshaped = demo[data_key][:].astype(np.float32).reshape(demo[data_key].shape[0], -1)
                    this_data.append(reshaped)
