...
class Denoiser(nn.Module):
    ...
    def loss(self, action, noise, sigma, **kwargs):
        """
        ...
        return F.mse_loss(model_output, target, reduction='none')


    def forward(self, action, sigma, **kwargs):
        """
        ...
        """
        c_skip, c_out, c_in = [append_dims(x, action.ndim) for x in self.get_scalings(sigma)]
        return self.inner_model(action * c_in, sigma, **kwargs) * c_out + action * c_skip
