...
class RandLogitNormal(Distribution):
    ...
    def sample(self, shape: List[int]) -> torch.Tensor:
        """Draws samples from a logit-normal distribution."""
        return torch.sigmoid(torch.randn(shape, device=self.device, dtype=self.dtype) * self.scale + self.loc)
