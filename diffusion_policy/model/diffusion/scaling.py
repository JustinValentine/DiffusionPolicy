...
class KarrasScaling(Scaling, torch.nn.Module):
    ...
    # TODO: BasePolicy does not have process batch method
    def calculate_sigma_data_of_action(
        self, policy: BasePolicy, dataloader: DataLoader
    ) -> float:
        """Computes the standard deviation of the action data."""
        actions = []
        for batch in dataloader:
            action, _, _ = policy.process_batch(batch)
